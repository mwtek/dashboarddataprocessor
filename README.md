...
<tr>
<td><a href="./files/Datensatzbeschreibung_Dashboard_v0_5_7_20260527.pdf" target="_blank">JSON dataset description</a></td>
...
```
git clone --branch v0.5.7+update.0 https://www.github.com/mwtek/utilities.git
git clone --branch v0.5.7+update.0 https://www.github.com/mwtek/dashboardlogic.git
git clone --branch v0.5.7+update.0 https://www.github.com/mwtek/dashboarddataprocessor.git
```
...
A precompiled file named `dashboarddataprocessor-0.5.7+update.0.jar` can be found in the project
...
```
java -jar target/dashboarddataprocessor-0.5.7+update.0.jar
```
...
necessary to have the `dashboarddataprocessor-0.5.7+update.0.jar` and the `application.yaml` you
want to use
on
...
```
COPY dashboarddataprocessor/target/dashboarddataprocessor-0.5.7+update.0.jar /dashboarddataprocessor/dashboard-data-processor.jar
...
file `DDP-v0.5.7+update.0.tar` is created, which contains the whole software. You can put it to any
server
...
```
http://localhost:9091/createJson?scopes=covid,influenza,kiradar,acribis,bct
```
...
documentation of the FHIR implementation</a> still needs to be updated to version 0.5.7+update.0.
...
```
java -jar -xMx 8G  target/dashboarddataprocessor-0.5.7+update.0.jar
```
